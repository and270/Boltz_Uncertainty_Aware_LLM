...
def load_and_prepare_models_for_eval():
    ...
    two_pass_tokenizer = AutoTokenizer.from_pretrained(CONFIG['output_dir'])

    # First, build the correct architecture on CPU
    base_model_for_eval = AutoModelForCausalLM.from_pretrained(CONFIG['output_dir'], torch_dtype=torch.float32, device_map={'':'cpu'})
    
    # Replace layers
    two_pass_model = replace_attention_layers(base_model_for_eval, base_model_for_eval.lm_head)
    
    # from_pretrained already loads weights, so we just move to device
